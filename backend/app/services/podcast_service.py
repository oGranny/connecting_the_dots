...
def _to_ssml(turns: List[Turn],
    ...
    # Multi-voice SSML for Azure
    # NOTE: Azure supports multiple <voice> blocks inside one <speak>.
    # Add subtle breaks between turns.
    parts = []
    for i, t in enumerate(turns):
        voice = voice_a if t.speaker == "A" else voice_b
        text = _sanitize(t.text)
        parts.append(
            f'<voice name="{voice}"><prosody rate="{rate}" pitch="{pitch}">{text}</prosody></voice>'
        )
        if i < len(turns) - 1:
            parts.append(f'<break time="{BREAK_MS_BETWEEN_TURNS}ms"/>')
    body = "".join(parts)
    # english india by default, adjust as you like
    return (
...
def build_transcript_from_selection(
    ...
    # lightweight source manifest for UI
    manifest = [
        {
            "rank": c.get("rank"),
